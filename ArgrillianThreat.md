namespace ArgrillianThreat
{
	...
	public class JobGiver_TendRetreatingAllies : ThinkNode_JobGiver
	{
		...
		protected override Job TryGiveJob(Pawn pawn)
		{
			...
			if (combatMedic)
			{
				// Always scan best candidates.
				// Then: if any downed ally is available, prefer rescuing that downed patient over
				// hauling / non-downed assigned targets (fixes "keeps hauling after healing" behavior).
				Pawn bestFromScan = FindBestPatient(pawn);

				Pawn assigned = medicComp.AssignedPawn;
				if (assigned == pawn) assigned = null; // never treat self as patient

				// If the scan found a downed patient, strongly prefer it.
				if (bestFromScan != null && bestFromScan.Downed)
				{
					patient = bestFromScan;
				}
				else
				{
					// Otherwise, prefer the assigned patient only if it's still urgent.
					if (assigned != null && !assigned.Dead && assigned.Map == pawn.Map && assigned.Faction == pawn.Faction)
					{
						if (pawn.Position.DistanceTo(assigned.Position) <= searchRadius)
						{
							float assignedHpPct = assigned.health.summaryHealth.SummaryHealthPercent;

							if (assignedHpPct <= 0.95f)
							{
								bool assignedBleeding = HasBloodLossStatic(assigned);
								float assignedBleedSeverity = BloodLossSeverityStatic(assigned);

								bool assignedUrgent =
									assigned.Downed ||
									assignedHpPct <= urgentTendHealthPercent ||
									(assignedBleeding && assignedBleedSeverity >= severeBleedHediffSeverity) ||
									assignedHpPct <= combatMedicInjuredHPPercentThreshold ||
									IsUrgentByScoreStatic(assigned, treatBelowHealthPercent, urgentScoreThreshold);
			...
			{
				// Non-combat medics: don't let assigned pawn be self either.
				Pawn assigned = medicComp.AssignedPawn;
				if (assigned == pawn) assigned = null;

				patient = assigned ?? FindBestPatient(pawn);
			}

			if (patient == null || patient.Dead) return null;
			if (patient.Map != pawn.Map) return null;

			// Only clear cached bed when we are NOT actively on medical for that patient and we are not carrying them.
			if (combatMedic)
			{
				bool medicHasMedicalJobForThisPatient =
					pawn.CurJob != null && ArgillianThreatPatientTuning.JobIsMedicalForPatient(pawn.CurJob, patient);

				bool medicCarryingThisPatient = IsMedicCarryingPawn(pawn, patient);

				if (!medicHasMedicalJobForThisPatient && !medicCarryingThisPatient)
			...
			bool patientUrgentByHPBleed =
				patient.Downed ||
				hpPct <= urgentTendHealthPercent ||
				(bleeding && bleedSeverity >= severeBleedHediffSeverity) ||
				(combatMedic && hpPct <= combatMedicInjuredHPPercentThreshold);

			bool patientUrgentByScore = IsUrgentByScoreStatic(patient, treatBelowHealthPercent, urgentScoreThreshold);
			bool patientUrgent = patientUrgentByHPBleed || patientUrgentByScore;

			bool hostilesPresent = disableTacticsIfHostilesPresent && HostilesPresentForMedic(pawn);

			// HARD OVERRIDE (fix "stand still until hostiles gone"):
			// If combat medic has a downed patient during hostiles and can tend now,
			// force a Tend/Rescue path immediately unless we're already medical-for-this-patient.
			if (combatMedic && hostilesPresent && patient.Downed && canTendNow(pawn, patient))
			{
				bool curIsMedicalForPatient =
					pawn.CurJob != null &&
					ArgillianThreatPatientTuning.JobIsMedicalForPatient(pawn.CurJob, patient);

				if (!curIsMedicalForPatient)
				{
					Pawn someoneElseNow = ArgillianThreatPatientTuning.PatientAlreadyBeingTendedOrRescuedBySomeoneElse(pawn, patient);
					if (someoneElseNow != null)
					{
						// Don't freeze; defend if patient is handled by someone else.
					...
					// Mark now so later ShouldWait gates don't immediately return null while we're committing.
					ArgrillianMedicalState.MedicTickCache.MarkNow(pawn);

					IntVec3 tendSpotForce = FindBestTendSpot(pawn, patient, radius: 6f);
					return ArgrillianGotoHelper.MakeGotoWithNoChurn(pawn, tendSpotForce);
				}
			}

			// NEW: if we're currently on a medicine-fetch job and we very recently started/kept a Tend task,
			// don't interrupt—otherwise we bounce between "go get medicine" and "go back to patient".
			bool recentlyStickingToTendTask = combatMedic && ArgrillianMedicalState.MedicTendTaskStickiness.RecentlyTookTendTask(pawn, tendTaskStickinessTicks);

			// NEW: Combat-medics: if patient is urgent (but NOT downed) and medic is in hostile behavior,
			// break away to tending spot (prevents "stays fighting -> patient never gets retreated/stopped").
			if (combatMedic && patientUrgent && !patient.Downed && pawn.CurJob != null && pawn.CurJob.def != null)
			{
				if (GetCarriedPawn(pawn) == null)
				{
					JobDef curDef = pawn.CurJob.def;
					...
					bool curIsHostileMovement =
						IsCombatAttackLikeJob(pawn.CurJob) ||
						IsChaseOrTacticJob(pawn.CurJob) ||
						IsFleeLikeJob(pawn.CurJob);

					if (!alreadyMedical && curIsHostileMovement)
					{
						pawn.jobs?.StopAll(true);
						ArgrillianMedicalState.MedicTickCache.MarkNow(pawn);

						// FIX: patient was not being locked at the moment we commit to tend pathing.
						// Lock/stabilize immediately so the patient doesn't keep retreating and break Tend.
						if (stopPatientWhenUrgent)
							TryStopPatientToAllowTendIfUrgentNonDowned(patient);

						LockPatientToMedic(pawn, patient);

						IntVec3 tendSpot = FindBestTendSpot(pawn, patient, radius: hostilesPresent ? 6f : 10f);
			...
			if (combatMedic && pawn.CurJob != null && pawn.CurJob.def != null)
			{
				...
				if (!isRelevantJobAlreadyForPatient)
				{
					...
					if ((patient.Downed && (medicIsHauling || medicIsMealOrConsume) && !IsMedicineFetchJob(pawn.CurJob)) ||
						(!patient.Downed && patientUrgent && (medicIsHauling || medicIsMealOrConsume) && !IsMedicineFetchJob(pawn.CurJob)))
					{
						pawn.jobs?.StopAll(true);
						ArgrillianMedicalState.MedicTickCache.MarkNow(pawn);
					}
			...
			if (combatMedic && pawn.CurJob != null && (patient.Downed || patientUrgent))
			{
				...
				{
					// Stop if we're doing a high-likelihood "drift" job type.
			...
			if (combatMedic && stopPatientWhenUrgent && patientUrgent && !patient.Downed && !patient.Dead)
			{
				if (pawn.CurJob != null && pawn.CurJob.def != null)
				{
					bool medicAlreadyDoingRelevant = pawn.CurJob.def == JobDefOf.TendPatient || pawn.CurJob.def == JobDefOf.Rescue;

					Pawn curPatient = ArgillianThreatPatientTuning.GetPatientFromJob(pawn.CurJob);

					bool medicApproachingThisPatient = (IsMoveLikeJob(pawn.CurJob)) && curPatient == patient;

					if (!medicAlreadyDoingRelevant && medicApproachingThisPatient)
						TryStopPatientToAllowTendIfUrgentNonDowned(patient);
				}
			}

			// Combat-medics: if downed patient and medic is doing something unrelated, push back to rescue/tend pathing.
			if (combatMedic && patient.Downed && pawn.CurJob != null && pawn.CurJob.def != null)
			{
				...
				bool isAllowedNow =
					curDef == JobDefOf.Rescue ||
					curDef == JobDefOf.TendPatient ||
					IsMoveLikeJob(pawn.CurJob);

				// IMPORTANT: chasing/hunting/retreat jobs can look "combat-related" but aren't attack-like.
				// Treat them as interruption-worthy when our patient is downed.
				bool curIsHostileMovement =
					IsCombatAttackLikeJob(pawn.CurJob) ||
					IsChaseOrTacticJob(pawn.CurJob) ||
					IsFleeLikeJob(pawn.CurJob);
				...
				if ((!isAllowedNow && curIsHostileMovement) || (!isAllowedNow && (medicIsHauling || medicIsMealOrConsume)))
				{
					pawn.jobs?.StopAll(true);
					ArgrillianMedicalState.MedicTickCache.MarkNow(pawn);
					ArgrillianMedicalState.RescueCommitCooldownCache.MarkCommit(pawn, patient);

					// FIX: ensure downed patient is locked immediately when medic switches to rescue/tend pathing.
					TryStopPatientToAllowTend(patient);
					LockPatientToMedic(pawn, patient);
			...
			if (combatMedic && patient.Downed && pawn.CurJob != null && pawn.CurJob.def != null)
			{
				...
				bool alreadyRelevant =
					curDef == JobDefOf.Rescue ||
					curDef == JobDefOf.TendPatient;

				bool mealJob = IsMealOrConsumeLikeJob(pawn.CurJob);

				if (!alreadyRelevant && mealJob)
				{
					pawn.jobs?.StopAll(true);
					ArgrillianMedicalState.MedicTickCache.MarkNow(pawn);
				}
			...
			if (someoneElse != null)
			{
				...
				{
					bool curIsStandOrWait =
						pawn.CurJob != null &&
						(pawn.CurJob.def == JobDefOf.Wait ||
							pawn.CurJob.def.defName == "StandStill" ||
							pawn.CurJob.def.defName == "Idle" ||
							pawn.CurJob.def.defName == "Stand");
