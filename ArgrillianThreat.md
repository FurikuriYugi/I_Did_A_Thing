namespace ArgrillianThreat
{
	...
	public class JobGiver_TendRetreatingAllies : ThinkNode_JobGiver
	{
		...
		medicCurJobDef == JobDefOf.TendPatient ||
		medicCurJobDef == JobDefOf.Rescue;
		...
		// At this point, the patient is held AND the owning medic is actively tending/rescuing.
			...
			if (retreatingHeldPatient && !tendEligibleNow)
			{
				...
				if (cur != null && cur.def != null)
				{
					...
					if (IsCombatAttackLikeJob(cur) || IsChaseOrTacticJob(cur))
					{
						...
						{
							bool los = GenSight.LineOfSight(pawn.Position, nearestHostile.Position, pawn.Map);
